...
每寫一章，工具就更了解你的故事——不是因為雲端模型或共用資料庫，而是因為你的故事記憶在本機持續累積：

- 章節越多 → RAG context 越豐富 → 審查越精準
- 角色設定越完整 → 一致性審查的誤報越少
- 伏筆追蹤器跨章節累積歷史，寫得越深，能回溯的範圍越遠
- 章節摘要層層疊加，形成整本稿件的壓縮記憶

寫到第 30 章，你的 Novel Assistant 已經熟悉你的世界觀、你的角色，以及所有尚未回收的伏筆。這份記憶完整保存在你的 `data/` 資料夾裡，隨你的故事走，不綁訂閱，不上雲端。
