...
<div class="card">
  ...
  <div class="grid-2">
    ...
    <div class="form-group">
      <label>相似度門檻 <span id="threshold-display">{{.RetrievalThreshold}}</span></label>
      <input type="range" id="retrieval-threshold" min="0" max="1" step="0.05"
             value="{{.RetrievalThreshold}}"
             oninput="document.getElementById('threshold-display').textContent=this.value;updateRetrievalSummary()">
    </div>
  </div>
...
function updateRetrievalSummary() {
  ...
  document.getElementById('retrieval-summary').textContent =
    '來源：' + srcStr + '　Top-K：' + opts.top_k + '　門檻：' + opts.threshold;
}
